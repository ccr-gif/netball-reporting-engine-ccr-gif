...
export default function MatchSetup({ onCreated }: { onCreated?: (matchId: string) => void }) {
  ...
  const usePreviousAll = async () => {
    ...
    try {
      ...
      if (prevCfg) {
        ...
        if (prevCfg.quarterDuration) setQuarterDuration(prevCfg.quarterDuration);
      }
    } catch (err: any) {
  ...
  const reset = () => {
    ...
    setDate(today); setTrackingCfg(makeDefaultConfig()); setQuarterDuration(15);
  };
  ...
  const save = async () => {
    ...
    try {
      ...
      let cfgToSave = normalizeMatchConfig({ ...trackingCfg, notes: notes.trim(), quarterDuration });
  ...
  return (
    <KeyboardAvoidingView style={{ flex: 1, backgroundColor: c.bg }} behavior={Platform.select({ ios: 'padding', android: undefined })} keyboardVerticalOffset={80}>
      <ScrollView style={{ flex: 1 }} contentContainerStyle={{ padding: 16, paddingBottom: 48 }} keyboardShouldPersistTaps="handled">
        ...
        <Text style={[s.label, { color: c.textSecondary }]}>Quarter duration (minutes)</Text>
        <View style={{ flexDirection: 'row', alignItems: 'center', gap: 10 }}>
          <TextInput
            value={String(quarterDuration)}
            onChangeText={t => {
              const n = parseInt(t.replace(/[^0-9]/g, ''), 10);
              if (!isNaN(n) && n > 0 && n <= 60) setQuarterDuration(n);
              else if (t === '') setQuarterDuration(0);
            }}
            keyboardType="number-pad"
            maxLength={2}
            style={[inp, { width: 72, textAlign: 'center', fontWeight: '900', fontSize: 20 }]}
          />
          <Text style={{ color: c.muted, fontWeight: '700' }}>minutes per quarter</Text>
        </View>
          ...
          {[10, 12, 15, 20].map(mins => (
            <Pressable
              ...
              style={[{ paddingHorizontal: 12, paddingVertical: 6, borderRadius: 8, borderWidth: 1 },
                { borderColor: quarterDuration === mins ? c.primary : c.cardBorder,
                  backgroundColor: quarterDuration === mins ? c.primary : 'transparent' }]}
              onPress={() => setQuarterDuration(mins)}
            >
              <Text style={{ fontWeight: '800', fontSize: 12, color: quarterDuration === mins ? '#fff' : c.muted }}>
                {mins}m
              </Text>
            </Pressable>
          ))}
