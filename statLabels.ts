...
import {
  listPlayers, getLineup, saveLineup, getPeriodScores, getCumulativeScores,
    ...
    <ScrollView contentContainerStyle={[mc.scroll, { backgroundColor: c.bg }]} bounces={false}>
      <View style={mc.container}>
        ...
        <View style={mc.qRow}>
          ...
          <Pressable style={[mc.smallBtn, { backgroundColor: c.primary }]} onPress={() => setLineupOpen(true)}>
        ...
        <View style={[mc.scoreCard, { backgroundColor: c.scoreBg, borderColor: c.cardBorder }]}>
          {[
            { name: totHome > totAway ? '🏆 ' : '', score: totHome, qScore: qHome, adj: (d: number) => d > 0 ? incHome(matchId!, period, d) : setScores(matchId!, period, Math.max(0, qHome+d), qAway), label: 'Home' },
            { name: '', score: totAway, qScore: qAway, adj: (d: number) => d > 0 ? incAway(matchId!, period, d) : setScores(matchId!, period, qHome, Math.max(0, qAway+d)), label: 'Away' },
          ].map((side, i) => (
